...
class LearnableSense_DCNet(torch.nn.Module):
    def __init__(
        self,
        model,
        model_config,
        ...
        self.sensenet = sensenet(**sense_config)
    ...
    def forwardGD(self, inputs):
        ...
        input_x = torch.cat((input_x[...,0],input_x[...,1]), 1)                          # [B, 30, 320, 320]
        csm = self.sensenet(input_x)                                                     # [B, 30, 320, 320]
        x0 = torch.sum(r2c(input_x * csm, axis=1), 1, keepdim=True)                      # [B, 1, 320, 320]
        x0 = c2r(x0, axis=1)                                                             # [B, 2, 320, 320]  -float32
    ...
    def forwardPM(self, inputs):
        ...
        input_x = torch.cat((input_x[...,0],input_x[...,1]), 1)                          # [B, 30, 320, 320]
        csm = self.sensenet(input_x)                                                     # [B, 30, 320, 320]
        x0 = torch.sum(r2c(input_x * csm, axis=1), 1)                                    # [B, 320, 320]
        x0 = c2r(x0, axis=1)                                                             # [B, 2, 320, 320]  -float32
        mask = random_sample_mask                                                        # [B, 320, 320]  -float32
